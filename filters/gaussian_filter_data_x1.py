...
count1 = sla_track1.count().item()
print(f"len of x={len(x)}, valid points={count1}")
sla_track_12 = sla_track1.rolling(points_numbers=12, min_periods=1, center=True).mean()
sla_track_np1 = sla_track1.values
sla_track_np = sla_track1.values
count = count_np(sla_track_np)
sla_track_np = fill_missing_nearest(sla_track_np)
sigma1 = 3
sla_track_ga = gaussian_filter1d(sla_track_np, sigma1)
...
print(x.shape)
plt.plot(x, sla_track_np1, label="raw")
...
plt.plot(x, sla_track_ga, label=f"guassian filter $\sigma$ = {sigma1}")
plt.legend()
plt.xlim(0,450)
info = ""
